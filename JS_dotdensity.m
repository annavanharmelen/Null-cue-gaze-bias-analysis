...
%                      ([1,0,0]) is accepted; default is 'b'
%
%       'lineWidth'    The width of the median or mean line
%
%   HDots = dotdensity(..) returns a vector of handles to dot density plots
%
...
%%
% Determine arguments and default values
...
%%
% Calculate new x coordinates and rearrange y coordinate
newX = zeros(size(y));
newY = zeros(size(y));
for ii = 1:length(x)
    yCol = y(:,ii);
...
% Draw dot density plot first time, just edges
hold on
lineCon = true;
if optionalArgs.lineCon
    for isub = 1:size(newX,1)
        line(newX(isub,:), newY(isub,:),'Color', [191/256,191/256,191/256],'LineWidth',1.7)
    end
end

for isub = 1:size(newX,1)
    for col = 1:size(newX,2)
        dotHandles(isub,col) = plot(ax, newX(isub,col), newY(isub,col), 'o');
    end

    % Set edge color of dots
    if ~isempty(optionalArgs.dotEdgeColor)
        setEdgeColor(optionalArgs.dotEdgeColor, dotHandles(isub,:))
    end

    % Set face color of dots
    if ~isempty(optionalArgs.dotFaceColor)
        setFaceColor(optionalArgs.dotFaceColor, dotHandles(isub,:))
    end

    % Set marker for dots
    if ~ isempty(optionalArgs.dotMarker)
        setMarker(optionalArgs.dotMarker, dotHandles(isub,:))
    end

    % Set dot size
    if optionalArgs.dotSize ~= 0
        setDotSize(optionalArgs.dotSize, dotHandles(isub,:))
    end
end
...
for ii = 1:length(x)
    ...
    % std_Error = std(yCol)./sqrt(length(yCol));
    ci=tinv(1-0.05/2,sum(~isnan(yCol))-1).*nanstd(yCol)./sqrt(sum(~isnan(yCol)));
    
    % ci = nanstd(yCol)./sqrt(sum(~isnan(yCol)));
    [l,p] = boundedline([x(ii) - spread, x(ii) + spread],[m,m], [ci ci],'cmap',[0.5, 0.5, 0.5],'alpha','transparency',.35);
    h(ii) = line([x(ii) - spread, x(ii) + spread],[m,m]);
...
addParameter(p,'medianLine',defaultMedianLine,@ischar);
addParameter(p,'meanLine',defaultMeanLine,@ischar);
addParameter(p,'delta',defaultDelta, @isscalar);
addParameter(p,'spread',defaultSpread, @isscalar);
addParameter(p,'nBins',defaultNBins, @isscalar);
addParameter(p,'labels',defaultLabels); 
addParameter(p, 'dotEdgeColor', defaultEdgeColor);
addParameter(p, 'dotFaceColor', defaultFaceColor);
addParameter(p, 'dotMarker',defaultMarker);
addParameter(p, 'dotSize', defaultDotSize);
addParameter(p, 'lineColor', defaultLineColor);
addParameter(p, 'lineWidth', defaultLineWidth);
addParameter(p, 'lineCon', defaultlineCon);
...
%% 
% Creates a set of x-coordinates for plotting one column of a dot density
